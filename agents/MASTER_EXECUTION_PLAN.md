...
- [x] **Sample Data**: Create 5+ Work Centers and 8+ Work Orders representing all 4 statuses.
...
- [x] **Interactive Hover**: Implement row-level background highlights (#F9FBFC) on hover.
...
- [ ] **Drawer Component**: Develop the slide-out `DetailsPanel` using Angular Animations (`transform: translateX`).
- [ ] **Reactive Forms**: Build the form using `FormGroup` for Name, Status, Start Date, and End Date.
- [ ] **Third-Party Integration**: Integrate `ngb-datepicker` and `ng-select`, styled to match Sketch symbols.
- [ ] **State Transformation**: Implement the "Pill-to-Text" visual change for the Status dropdown (Pill in selection, Plain text in list).
- [ ] **Error Handling**: Add UI feedback (Toast/Banner) if an overlap is detected during Save/Create.

## Phase 5: QA, Performance & Submission

- [ ] **Pixel-Perfect Audit**: Verify all dimensions, colors, and shadows against Sketch "Inspect" values.
- [x] **Optimization**: Implement `OnPush` change detection and `trackBy` for timeline rendering performance.
- [ ] **Persistence (Bonus)**: Implement `localStorage` to persist Work Order changes across refreshes.
